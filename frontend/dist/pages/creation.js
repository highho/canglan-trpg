...
let sel = { race: '', clazz: '', trait: '', difficulty: 'NORMAL' };
export function initCreationPage() {
...
export function enterCreation() {
    ...
    const name = document.getElementById('creation-name');
    if (name)
        name.value = '';
    setPage('creation');
    void refresh();
...
async function refresh() {
    try {
        ...
        renderGroup('trait-choices', opts.traits.map(o => o.name), sel.trait, pickTrait, opts.traits.map(o => o.description ?? ''));
        ...
        if (summary)
            summary.textContent = `选项加载失败：${err.message}`;
    }
}
function pickRace(name) { sel.race = name; sel.trait = ''; void refresh(); }
function pickClass(name) { sel.clazz = name; sel.trait = ''; void refresh(); }
function pickTrait(name) { sel.trait = name; void refresh(); }
/**
 * 渲染一组选卡（html 拼接整块重绘；descs 提供时写入 title 提示）。
 * 选卡点击：数据挂 data-pick 走容器委托（页面级，见下方 initCreationPage 注册）。
 */
function renderGroup(containerId, names, selected, onPick, descs) {
...
function renderDifficulty(difficulties) {
    ...
    if (text)
        text.textContent = `难度：${DIFF_CN[sel.difficulty] ?? sel.difficulty}。${DIFF_DESC[sel.difficulty] ?? ''}`;
}
/** 摘要行（全文字描述）。 */
function renderSummary() {
    ...
    if (!summary)
        return;
    const name = document.getElementById('creation-name')?.value.trim() || '旅人';
    const parts = [`名字：${name}`];
    if (sel.race)
        parts.push(`血脉：${sel.race}`);
    if (sel.clazz)
        parts.push(`道路：${sel.clazz}`);
    if (sel.trait)
        parts.push(`特质：${sel.trait}`);
...
async function beginAdventure() {
    ...
    if (!sel.race || !sel.clazz || !sel.trait) {
        if (summary)
            summary.textContent = '血脉、道路和出身特质得选齐了才能上路';
        return;
    }
    try {
        ...
        if (!resp.sessionId)
            throw new Error('服务器没给会话');
        setState({ sessionId: resp.sessionId, hud: resp.hud });
        // 建档自动存档落在槽位 1；选了其他槽位则补一次显式存档
        const slot = getState().saveSlot;
        if (slot !== 1)
            await api.save(resp.sessionId, slot).catch(() => undefined);
        await enterGame();
    }
    catch (err) {
        if (summary)
            summary.textContent = `建档失败：${err.message}`;
    }
}
/** 属性值转义（title/按钮内容用）。 */
function escAttr(s) {
