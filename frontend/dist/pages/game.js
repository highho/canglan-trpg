...
export function initGamePage() {
    ...
    // 底部八菜单（静态骨架，直接绑定）
    document.querySelectorAll('.menus .menu').forEach(btn => {
    ...
    document.getElementById('log-toggle')?.addEventListener('click', () => {
        ...
        if (toggle)
            toggle.textContent = open ? '事件日志（点此收起）' : '事件日志（点此展开）';
    });
    // 小地图：打开（本地缓存渲染）/ 关闭
    document.getElementById('btn-mini-map')?.addEventListener('click', () => {
        ...
        if (body)
            body.innerHTML = mapCache ? renderMiniMap(mapCache) : '<div class="ov-muted">地图还没铺开，稍后再看。</div>';
    });
    ...
    if (filterBar) {
        filterBar.addEventListener('change', ev => {
            ...
            if (!box)
                return;
            if (box.value === '资源')
                filterState.resource = box.checked;
            if (box.value === '怪物')
                filterState.monster = box.checked;
            if (box.value === 'NPC')
                filterState.npc = box.checked;
            redrawLocalMap();
        });
        filterBar.addEventListener('input', ev => {
            const box = ev.target.closest('input[type="text"]');
            if (!box)
                return;
...
/** 标记在当前过滤下是否显示。 */
function markVisible(mark) {
    const group = FILTER_GROUP[mark];
    if (!group)
        return true; // 建/门/家不分组，常显
    if (group === '资源')
        return filterState.resource;
    if (group === '怪物')
        return filterState.monster;
...
/** 用缓存数据重绘局部地图（过滤条变化时）。 */
function redrawLocalMap() {
    if (mapCache)
        renderLocalMap(mapCache);
}
/** 局部地图：以玩家为中心渲染 9×9 格子，四正相邻格可点击移动（按过滤条显示）。 */
function renderLocalMap(d) {
...
export async function enterGame() {
    ...
    try {
        ...
        if (narration)
            appendNarration(narration, state.log);
        setState({ hud: state.hud });
        renderHud(state.hud);
    }
    catch (err) {
        if (narration)
            appendNarration(narration, [{ text: `恢复游戏状态失败：${err.message}`, kind: 'ERROR' }]);
    }
...
export async function send(line) {
    ...
    try {
        ...
        if (narration)
            appendNarration(narration, resp.narration);
        renderHud(resp.hud);
    }
    catch (err) {
        if (narration)
            appendNarration(narration, [{ text: `指令失败：${err.message}`, kind: 'ERROR' }]);
    }
    await refreshLocalMap();
}
