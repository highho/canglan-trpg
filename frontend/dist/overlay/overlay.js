...
export function initOverlay(onSave) {
    ...
    document.addEventListener('click', ev => {
        const btn = ev.target.closest('[data-cx-cat]');
...
export function openOverlay(name) {
    ...
    void (async () => {
        try {
            ...
            // 图鉴输入框绑定（每次重绘后重建）
            const filterBox = document.getElementById('codex-filter');
            if (filterBox) {
            ...
            body.innerHTML = `<div class="ov-muted">面板打不开：${esc(err.message)}</div>`;
        }
    })();
}
...
function renderChar(d) {
    ...
    if (equips.length) {
        ...
        for (const e of equips)
            html += info(`${e.slot}：${e.name}（耐久 ${e.durability}）`);
    }
...
/* ═══ 技能（通用列表） ═══ */
function renderList(d, key, head, cmds) {
    let html = '';
    html += title(head);
    const names = d[key] ?? [];
...
function renderQuest(d) {
    ...
    if (d['hint'])
        html += info(String(d['hint']));
    return html;
}
/* ═══ 家园 ═══ */
function renderHome(d) {
    if (!d['hasHome'])
        return info('连个窝都没有，先攒点家底吧。');
    let html = '';
...
function drawCodex() {
    ...
    if (nav) {
        ...
        for (const c of cats) {
            nav.insertAdjacentHTML('beforeend', `<button data-cx-cat="${c.key}" class="${c.key === codexCat ? 'selected' : ''}">${c.label}</button>`);
        }
    }
    const rows = codexData[codexCat] ?? [];
