...
import type { Hud, NarrationLine } from '../net/api.js';
import { esc } from './store.js';

/** 生态枚举 → 中文名。 */
export const BIOME_NAME: Record<string, string> = {
  PLAINS: '平原', FOREST: '林地', DESERT: '沙漠',
  TUNDRA: '苔原', SWAMP: '沼泽', MOUNTAIN: '山地',
};

/** 难度 → 中文（DifficultyMode 五档）。 */
export const DIFF_CN: Record<string, string> = {
  CASUAL: '休闲', NORMAL: '普通', HARD: '困难', NIGHTMARE: '梦魇', ABYSS: '深渊',
};

/** 难度描述（口语化）。 */
export const DIFF_DESC: Record<string, string> = {
  CASUAL: '粮草充足，躺着也能玩。',
...
export function appendNarration(container: HTMLElement, lines: NarrationLine[]): void {
  let html = '';
...
export function renderHud(hud: Hud): void {
  ...
  if (overload) overload.hidden = !hud.overloaded;

  setHtml('hud-vitals',
    attr('生命', `${hud.hp ?? 0}/${hud.maxHp ?? 0}`) +
...
function renderMovePad(hud: Hud): void {
  const dirs = hud.directions ?? [];
  document.querySelectorAll<HTMLButtonElement>('.move-pad .pad-btn[data-cmd]').forEach(btn => {
...
function renderNearby(hud: Hud): void {
  ...
  if (!block || !cards || !dialogBlock) return;
...
function renderQuickBar(hud: Hud): void {
