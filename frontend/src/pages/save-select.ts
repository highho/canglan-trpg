...
export async function enterSaveSelect(): Promise<void> {
  ...
  if (title) title.textContent = mode === 'new' ? '选择存档位' : '读取存档';
  ...
  try {
    ...
    wrap.innerHTML = `<div class="ov-muted">读存档位失败：${esc((err as Error).message)}</div>`;
  }
  setPage('saveSelect');
}

async function pickSlot(slot: number, hasSave: boolean): Promise<void> {
  const mode = getState().saveMode;
  if (mode === 'new') {
