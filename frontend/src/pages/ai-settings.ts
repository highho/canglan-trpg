...
export async function openAiSettings(): Promise<void> {
  ...
  try {
    ...
    setStatus(`读配置失败：${(err as Error).message}`, 'err');
  }
}
...
function renderBody(body: HTMLElement): void {
  ...
  body.querySelectorAll<HTMLButtonElement>('[data-preset]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      body.querySelectorAll<HTMLButtonElement>('[data-preset]').forEach(b => b.classList.toggle('selected', b === btn));
    });
  });
...
function fill(cfg: AiProviderConfig): void {
  ...
  document.querySelectorAll<HTMLButtonElement>('#ai-presets [data-preset]').forEach(btn => {
...
async function onTest(): Promise<void> {
  const cfg = collect();
  if (!cfg.baseUrl) { setStatus('先把服务地址填上。', 'err'); return; }
  setStatus('正在试连供应商……', '');
  try {
    const r = await api.testAi(cfg);
    if (r.ok) setStatus(`连上了，模型回话：${r.reply ?? ''}`, 'ok');
    else setStatus(`试连没成：${r.error ?? '不知道啥毛病'}`, 'err');
  } catch (err) {
    setStatus(`试连没成：${(err as Error).message}`, 'err');
  }
}

async function onSave(): Promise<void> {
  const cfg = collect();
  if (cfg.enabled && !cfg.baseUrl) { setStatus('要启用就得先填服务地址。', 'err'); return; }
  try {
